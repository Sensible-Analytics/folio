...
the main Sensible Folio application and addons, enabling automatic cache
...
(window as any).__sensible_folio_query_client__ = queryClient;
```
...
```typescript
// Shared in @sensible-folio/addon-sdk
...
```typescript
import { QueryKeys } from "@sensible-folio/addon-sdk";
```
...
components of the Sensible Folio ecosystem.
