...
impl<E: AiEnvironment + 'static> Tool for GetHoldingsTool<E> {
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let holdings_dto: Vec<HoldingDto> = holdings
            .into_iter()
            .filter(|h| h.holding_type != sensible_folio_core::holdings::HoldingType::Cash)
            .take(MAX_HOLDINGS)
            .map(|h| {
                ...
                let holding_type = match h.holding_type {
                    sensible_folio_core::holdings::HoldingType::Cash => "Cash",
                    sensible_folio_core::holdings::HoldingType::Security => "Security",
