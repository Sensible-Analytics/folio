...
use sensible_folio_core::errors::{DatabaseError, Error};
use thiserror::Error;
...
/// `sensible_folio_core::Error` before being returned to callers.
#[derive(Error, Debug)]
...
pub trait IntoCore<T> {
    fn into_core(self) -> sensible_folio_core::Result<T>;
}
