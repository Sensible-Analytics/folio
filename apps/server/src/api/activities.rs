...
#[serde(untagged)]
enum SortWrapper {
    One(sensible_folio_core::activities::Sort),
    Many(Vec<sensible_folio_core::activities::Sort>),
}
...
async fn search_activities(
    ...
    let sort_normalized: Option<sensible_folio_core::activities::Sort> = match body.sort {
...
async fn parse_csv_endpoint(
    ...
    let result = sensible_folio_core::activities::parse_csv(&content, &config)?;
    Ok(Json(result))
}
