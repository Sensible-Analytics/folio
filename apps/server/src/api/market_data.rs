...
use sensible_folio_market_data::ExchangeInfo;
...
) -> ApiResult<Json<sensible_folio_core::quotes::ResolvedQuote>> {
    let inst_type = q
        .instrument_type
        .as_deref()
        .and_then(sensible_folio_core::assets::InstrumentType::from_db_str);
    let res = state
        .quote_service
...
async fn get_exchanges() -> Json<Vec<ExchangeInfo>> {
    Json(sensible_folio_market_data::get_exchange_list())
}
